...
@section('content')
    <div class="content">
        <div class="intro-y flex items-center mt-6">
            <h2 class="text-lg font-medium mr-auto">{{ $announcement->title }}</h2>
        </div>
                ...
                <div class="intro-y box mt-5">
                    <div class="relative flex items-center p-5">
                        ...
                        <div class="ml-4 mr-auto">
                            <div class="font-medium text-base">Announcement #{{ $announcement->id_formatted }}</div>
                            <div class="text-slate-500">Submitted {{ $announcement->submitted_at->diffForHumans() }}</div>
                        </div>
                    </div>
                    ...
                    @if(\App\Classes\PermissionChecker::instance()->hasPermission(['manage_categories_edit', 'manage_categories_delete']))
                    <div class="p-5 border-t border-slate-200/60 dark:border-darkmode-400 flex">
                        <div class="flex items-center justify-center">
                            @if(\App\Classes\PermissionChecker::instance()->hasPermission('manage_categories_edit'))
                            <a href="{{ route('announcements.edit', ['announcement' => $announcement->id]) }}" class="text-primary flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            ...
                            @endif
                                @if(\App\Classes\PermissionChecker::instance()->hasPermission('manage_categories_delete'))
                            ...
                            <div class="flex-1 mt-6 xl:mt-0">
                                <div class="grid grid-cols-12 gap-x-5">
                                    <div class="col-span-12 xl:col-span-12 mb-4">
                                        ...
                                        <div class="flex flex-col sm:flex-row mt-2">
                                            <div class="form-check mr-5 mt-2 sm:mt-0">
                                                <input id="is_active-1" class="form-check-input" type="radio" name="is_active" value="1" @if($announcement->is_active) checked @endif disabled>
                                                <label class="form-check-label" for="is_active-1">Active</label>
                                            </div>
                                            <div class="form-check mr-5">
                                                <input id="is_active-2" class="form-check-input" type="radio" name="is_active" value="0" @if(!$announcement->is_active) checked @endif disabled>
                                    ...
                                    <div class="col-span-12 xl:col-span-12 mb-4">
                                        ...
                                        <input id="title" type="text" class="form-control w-full" name="title" value="{{ $announcement->title }}" disabled>
                                    </div>
                                    <div class="col-span-12 xl:col-span-12 mb-4">
                                        <label for="message" class="form-label">Message</label>
                                        <textarea name="message" id="message" cols="30" rows="5" class="form-control w-full" disabled>{{ $announcement->message }}</textarea>
                                    </div>
                                    ...
                                    <div class="col-span-6 xl:col-span-6 mb-4">
                                        ...
                                        <input id="submitted_at" type="date" class="form-control w-full" name="submitted_at" value="{{ $announcement->submitted_at->format('Y-m-d') ?? date('Y-m-d') }}" disabled>
                                    </div>
                                </div>
                            </div>
                        </div>
