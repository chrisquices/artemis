<div class="content">
    <div class="@if($view_type == 'grid') grid @endif grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center">
            ...
            @if(\App\Classes\PermissionChecker::instance()->hasPermission('manage_categories_create'))
            <a href="{{ route('categories.create') }}" class="btn btn-primary shadow-md xs-mt-4 xs-w-100">Add New Category</a>
            @endif
        </div>
                        ...
                        <div class="flex items-start px-5 pt-5">
                            <div class="w-full flex flex-col lg:flex-row items-center">
                                ...
                                <div class="lg:ml-4 text-center lg:text-left mt-3 lg:mt-0">
                                    <a href="javascript:void(0);" class="font-medium">{{ $category->name }}</a>
                                </div>
                            </div>
                        </div>
                        <div class="text-center lg:text-left p-5"></div>
                        <div class="text-center lg:text-right p-5 border-t border-slate-200/60 dark:border-darkmode-400">
                            @if($category->is_active)
                                <div class="flex items-center justify-center text-primary">
                                    ...
                                    @if(\App\Classes\PermissionChecker::instance()->hasPermission('manage_categories_show'))
                                        <a class="flex items-center ml-5 text-primary" href="{{ route('categories.show', ['category' => $category->id]) }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                                                 stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <tr class="cursor-pointer">
                        ...
                        <td class="table-report__action w-40">
                            <div class="flex justify-center items-center text-primary">
                                <a class="flex items-center mr-3" href="{{ route('categories.show', ['category' => $category->id]) }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                                         stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                         icon-name="search" data-lucide="search" class="lucide lucide-activity w-4 h-4 mr-1">
                                        <polyline points="9 11 12 14 22 4"></polyline>
        ...
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center" wire:loading.remove>
            <div class="hidden md:block mr-auto text-slate-500">
                @if($categories->total() > 0)
                    Showing {{ $categories->count() * ($categories->currentPage() - 1) + 1 }}-{{ $categories->count() }} of {{ $categories->total() }} entries
                @else
                    Showing 0-0 of {{ $categories->total() }} entries
                @endif
            </div>

            {{ $categories->links('layouts.custom-pagination') }}
