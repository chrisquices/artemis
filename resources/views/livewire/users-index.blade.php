<div class="content">
    <div class="@if($view_type == 'grid') grid @endif grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap  items-center">
            ...
            @if(\App\Classes\PermissionChecker::instance()->hasPermission('manage_users_create'))
            <a href="{{ route('users.create') }}" class="btn btn-primary shadow-md xs-mt-4 xs-w-100">Add New User</a>
            @endif
        </div>
                    ...
                    <div class="box">
                        <div class="flex items-start px-5 pt-5">
                            <div class="w-full flex flex-col lg:flex-row items-center">
                                ...
                                <div class="lg:ml-4 text-center lg:text-left mt-3 lg:mt-0">
                                    <a href="javascript:void(0);" class="font-medium">{{ $user->name }}</a>
                                    <div class="text-slate-500 text-xs mt-0.5">{{ $user->type }}</div>
                                </div>
                            </div>
                        </div>
                        <div class="text-center lg:text-left p-5">
                            ...
                            <div class="flex items-center justify-center lg:justify-start text-slate-500 mt-1">
                                ...
                                {{ $user->projectUsers->count() }} Projects, {{ $user->tickets->count() }}  Tickets
                            </div>
                        </div>
                        <div class="text-center lg:text-right p-5 border-t border-slate-200/60 dark:border-darkmode-400">
                            @if($user->is_active)
                                <div class="flex items-center justify-center text-primary">
                                    ...
                                    <a class="flex items-center ml-5 text-primary" href="{{ route('users.show', ['user' => $user->id]) }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
               ...
               <table class="table table-report py-4" wire:loading.remove>
                ...
                @foreach($users as $user)
                    <tr class="cursor-pointer">
                        <td class="w-20">
                            <img src="{{ $user->photo_url }}" alt="Logo" class="rounded-full ml-1" width="20">
                        </td>
                        <td>{{ $user->type }}</td>
                        <td>{{ $user->name }}</td>
                        <td>{{ $user->email }}</td>
                        ...
                        <td class="table-report__action w-40">
                            <div class="flex justify-center items-center text-primary">
                                <a class="flex items-center mr-3" href="{{ route('users.show', ['user' => $user->id]) }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                                         stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                         icon-name="search" data-lucide="search" class="lucide lucide-activity w-4 h-4 mr-1">
                                        <polyline points="9 11 12 14 22 4"></polyline>
        ...
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center" wire:loading.remove>
            <div class="hidden md:block mr-auto text-slate-500">
                @if($users->total() > 0)
                    Showing {{ $users->count() * ($users->currentPage() - 1) + 1 }}-{{ $users->count() }} of {{ $users->total() }} entries
                @else
                    Showing 0-0 of {{ $users->total() }} entries
                @endif
            </div>
