...
@section('content')
    <div class="content">
        <div class="intro-y flex items-center mt-6">
            <h2 class="text-lg font-medium mr-auto">{{ $status->name }}</h2>
        </div>
                ...
                <div class="intro-y box mt-5">
                    <div class="relative flex items-center p-5">
                        ...
                        <div class="ml-4 mr-auto">
                            <div class="font-medium text-base">{{ $status->name }}</div>
                        </div>
                    </div>
                    ...
                    @if(\App\Classes\PermissionChecker::instance()->hasPermission(['manage_statuses_edit', 'manage_statuses_delete']))
                        <div class="p-5 border-t border-slate-200/60 dark:border-darkmode-400 flex">
                            <div class="flex items-center justify-center">
                                @if(\App\Classes\PermissionChecker::instance()->hasPermission('manage_statuses_edit'))
                                ...
                                @endif
                                @if(\App\Classes\PermissionChecker::instance()->hasPermission('manage_statuses_delete'))
                                    <form action="{{ route('statuses.delete', ['status' => $status->id]) }}" method="POST"
                            ...
                            <div class="flex-1 mt-6 xl:mt-0">
                                <div class="grid grid-cols-12 gap-x-5">
                                        ...
                                        <div class="flex flex-col sm:flex-row mt-2">
                                            <div class="form-check mr-5 mt-2 sm:mt-0">
                                                <input id="is_active-1" class="form-check-input" type="radio" name="is_active" value="1"
                                                       @if($status->is_active) checked @endif disabled>
                                    ...
                                    <div class="col-span-12 xl:col-span-6 mb-4">
                                        ...
                                        <input id="name" type="text" class="form-control w-full" name="name" value="{{ $status->name }}"
                                               disabled>
                                    </div>
                                    <div class="col-span-12 xl:col-span-6 mb-4">
                                        <label for="type" class="form-label">Type</label>
                                        <input id="type" type="text" class="form-control w-full" name="type" value="{{ $status->type }}"
                                               disabled>
                                    </div>
                                    <div class="col-span-12 xl:col-span-6 mb-4">
                                        <label for="color" class="form-label">Color</label>
                                        <input id="color" type="text" class="form-control w-full" name="color" value="{{ $status->color }}" disabled>
                                    </div>
                                </div>
                            </div>
                        </div>
